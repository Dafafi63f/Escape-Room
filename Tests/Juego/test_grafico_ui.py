...
class TestTextosUi(unittest.TestCase):
    ...
    def test_emoji_icono_ranking(self, _mock: object) -> None:
        ...
        self.assertEqual(emoji_icono("ranking"), "🏆")
...
from Comun.jugador import NOMBRE_JUGADOR_DEFECTO  # noqa: E402
...
# --- test_limpieza_local.py ---

from Comun.limpieza_local import (  # noqa: E402
    borrar_txt_informes_feedback,
    listar_txt_informes_feedback,
    vaciar_ranking_variante,
)
from Comun.ranking_resistencia import top_records


class TestLimpiezaLocal(unittest.TestCase):
    def test_borrar_txt_informes_y_feedback(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            informes = Path(tmp) / "Informes"
            feedback = Path(tmp) / "Feedback"
            informes.mkdir()
            feedback.mkdir()
            (informes / "a.txt").write_text("a", encoding="utf-8")
            (feedback / "b.txt").write_text("b", encoding="utf-8")
            with (
                patch("Comun.limpieza_local.resolver_dir_informes", return_value=informes),
                patch("Comun.limpieza_local.resolver_dir_feedback", return_value=feedback),
            ):
    ...
    def test_vaciar_ranking_variante(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "Comun.limpieza_local.path_ranking_para_variante",
                return_value=path,
            ):
