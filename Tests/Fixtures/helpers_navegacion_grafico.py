...
def datos_juego_completos() -> DatosJuego:
    """Carga real del banco (pool completo) para flujos UI de modos especiales."""
...
class SecuenciaNavegacion:
    ...
    def pulsar_modo_especial(self, preset_id: str) -> Boton:
        from Grafico.pantallas_modos import ConfigModosEspeciales

        self.comprobar(ConfigModosEspeciales)
        for boton, preset in zip(
            self.pantalla.botones_modo, self.pantalla.presets, strict=True
        ):
            if preset.id == preset_id:
                pulsar(boton)
                return boton
        visibles = [p.id for p, b in zip(self.pantalla.presets, self.pantalla.botones_modo, strict=True) if b.activo]
        raise AssertionError(
            f"Preset especial {preset_id!r} no encontrado. Activos: {visibles}"
        )
