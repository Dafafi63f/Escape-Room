...
`Tests/Fixtures/support.py` configura `sys.path` hacia `Juego/`. Cada módulo `test_*.py` debe llamar a `ensure_juego_path()` antes de importar `Comun` o `Grafico`.
...
- **Run Integration Tests** — `Tests/` + `Files/mantenimiento.py validar` + `health_check.py`
...
| `test_carga_contenido.py` | Perfil de contenido, paquete mínimo/completo y `--csv` |
