...
\smallskip
\noindent\textbf{Versión de entrega:} dos Word en \texttt{Entrega/} (\texttt{Memoria\_TFG\_markdown.docx}, \texttt{Memoria\_TFG\_latex.docx}). Regenerar: \texttt{python Entrega/exportar\_memoria.py}. El PDF de entrega lo exportas desde Word tras editar.
...
\begin{itemize}
  \item gestionar un banco de preguntas \textbf{estructurado y auditable} (por el autor y el profesorado, mediante reglas automatizadas, revisión manual documentada y scripts de mantenimiento reproducibles),
  ...
  \item y sentar las bases para modelos pedagógicos más ricos.
\end{itemize}

\textbf{Gamificación:} elementos de diseño de juego (puntos, vidas, retos) en contexto formativo (Deterding et al., 2011). \textbf{Banco auditable:} ítems revisables de forma sistemática con \texttt{mantenimiento.py validar}, \texttt{Revision/revision\_manual\_banco.md} y auditorías. \textbf{Distractores plausibles:} opciones incorrectas creíbles para quien no domina el concepto (Haladyna et al., 2002). \textbf{Narrativa} (capa pendiente): secuencia ficcional de escenas/salas; el entregable actual funciona sin ella. \textbf{Multiasignatura} y \textbf{prerrequisitos}: solapamiento entre materias y conocimientos previos asumidos.
...
\begin{itemize}
  ...
  \item Implementar un \textbf{modo historia} que genere exámenes balanceados según el histórico de calificaciones del grado (\texttt{Historic\_qualificacions\_MatCAD\_completo.csv}).
\end{itemize}
...
\textbf{Procedimiento} (implementado en \texttt{generador\_examen\_historia.py}):
\begin{enumerate}
  \item Por cada materia: media numérica, tasa de suspensos (nota $< 5$) e \textbf{índice de dificultad} (0..1) combinando ambos indicadores.
  \item El generador asigna \textbf{pesos} según el perfil (p.\ ej.\ \textit{refuerzo} incrementa el peso de materias con índice alto).
  \item Por materia seleccionada, se rellenan \textit{slots} canónicos Teoría/Cálculo $\times$ Fácil/Media/Difícil con preguntas del banco cerrado.
\end{enumerate}
...
\texttt{simulacion\_evaluacion\_azar.py} ejecuta \textbf{50\,000} partidas de \textbf{20} preguntas con respuesta uniforme A--D (25\,\% teórico de acierto). Resultados: nota media examen \textbf{2,5}/10; aciertos \textbf{24,98\,\%}; en modo arcade con 3 vidas, \textbf{100\,\%} de partidas agotan vidas (media 4 preguntas); puntos medios \textbf{$-10$}. El azar no permite aprobar ni completar bloques largos.
...
\midrule
\textbf{H1} & Apoyada: filtros curriculares operativos \\
\textbf{H2} & Apoyada: auditorías con conteos reproducibles \\
\textbf{H3} & Apoyada: histórico 8818 registros; véase \S5.6 \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  ...
  \item \textbf{Motivación y gamificación:} contrastar mecánicas de juego frente a listado estático (piloto con usuarios, SUS); no hipótesis contrastada en este TFG.
