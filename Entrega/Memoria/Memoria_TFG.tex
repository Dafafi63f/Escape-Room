...
\midrule
Lanzadores & \texttt{juego\_consola.py}\newline\texttt{juego\_grafico.py} & Terminal y pygame (prototipo modo libre) \\
...
Pruebas de regresión & \texttt{python -m unittest discover -s Tests -v} (74 tests) \\
...
Lanzador gráfico & \texttt{Juego/juego\_grafico.py} + \texttt{Juego/Grafico/} (prototipo modo libre) \\
Paquete de lógica & \texttt{Juego/Comun/} (dominio) + \texttt{Juego/Consola/} (UI terminal) \\
...
Pruebas & Suite en \texttt{Tests/} --- \textbf{74 tests} (\texttt{Tests/Juego/}, \texttt{Tests/Scripts/}); CI en GitHub Actions \\
\bottomrule
\end{tabular}
...
OE4 & Parcial (prototipo) & \texttt{juego\_grafico.py} + modo libre pygame; consola sigue siendo referencia completa \\
OE5 & \textbf{Parcialmente cumplido} & Banco validado, 74 tests + CI; sin estudio con usuarios \\
\bottomrule
\end{tabular}
\fintabla

Los \textbf{objetivos específicos} OE2, OE3 y OE5 están cubiertos en su versión de consola. OE4 avanza con un prototipo gráfico (modo libre en pygame); OE1 (narrativa gráfica completa) queda como trabajo futuro. Esta decisión es defendible: el prototipo valida el núcleo evaluable antes del coste de la capa narrativa visual, en línea con el principio de prototipado incremental en ingeniería del software.
...
H & Estado del proyecto y seguimiento & \texttt{CHANGELOG.md}, \texttt{CHECKLIST.md} \\
