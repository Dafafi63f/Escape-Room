...
| [`utilidades_tfg.py`](utilidades_tfg.py) | Memoria Word, limpieza, `.exe` y zip portable (incremental: reutiliza si no hay cambios) |
...
```bash
python Docs/utilidades_tfg.py                 # figuras + Word + .exe → limpieza → zip (incremental)
python Docs/utilidades_tfg.py --solo-memoria  # figuras + Word si hace falta
python Docs/utilidades_tfg.py --solo-figuras  # solo PNG
python Docs/utilidades_tfg.py --sin-exe       # sin compilar .exe
python Docs/utilidades_tfg.py --sin-figuras    # reutilizar PNG existentes
```

**Incremental:** figuras (solo gráficos con datos), Word, `.exe` y zip se omiten si sus entradas no cambiaron.
Arquitectura, pipeline historia y comparación Inka ↔ TFG son **tablas** en la memoria, no PNG.
Forzar: `--forzar-figuras`, `--forzar-memoria`, `--forzar-exe`, `--forzar-zip`.
