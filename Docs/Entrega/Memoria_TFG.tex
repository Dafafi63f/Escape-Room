...
Sea $s$ el número de salas consecutivas sin haber visto el evento. La probabilidad en el siguiente intento es
\[
...
Sin pity ($\delta=0$), la probabilidad de no ver descanso en $n=30$ salas es $(1-p_0)^n \approx 15{,}6\,\%$; la racha p95 sin evento alcanza 29 salas (10\,000 réplicas). Con pity suave + hard pity en la sala 5, ninguna réplica termina sin descanso; la racha p95 queda en 4 salas y la sala media del primer descanso es \textbf{4,13} (figuras~\ref{fig:pity-curva}--\ref{fig:pity-dist}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{pity_curva_probabilidad.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{pity_comparacion_descanso.png}
\caption{Modelo simplificado (10\,000 réplicas): comparación con y sin pity.}
\label{fig:pity-comp}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{pity_distribucion_primer_descanso.png}
\caption{Distribución de la sala del primer descanso (modelo con pity).}
\label{fig:pity-dist}
\end{figure}

El script \texttt{Files/simulacion\_pity.py} reproduce el modelo; las figuras se generan con \texttt{python Docs/generar\_figuras\_memoria.py}.
...
OE5 & Valor formativo & \textbf{Parcialmente cumplido} & Banco validado, 424 tests + CI; sin estudio con usuarios \\
\bottomrule
\end{tabular}
\fintabla
