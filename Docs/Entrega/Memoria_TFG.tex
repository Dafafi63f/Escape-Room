...
\noindent\textit{Nota sobre el título:} el documento de proyecto inicial planteaba un escape room con interfaz gráfica. El \textbf{entregable de este TFG} es el cuestionario (banco + modos de juego en consola y gráfico pygame) descrito en esta memoria; la capa narrativa escape room completa queda como evolución futura del mismo proyecto.
...
\midrule
Lanzadores & \texttt{juego\_consola.py}\newline\texttt{juego\_grafico.py} & Terminal y pygame (libre, historia, resistencia, feedback) \\
Dominio & \texttt{Juego/Comun/} (\texttt{motor\_nucleo.py}, \texttt{reglas\_partida.py}, \texttt{datos.py}, \ldots) & Reglas, puntuación, pool, rutas compartidas \\
Modos (consola) & \texttt{modo\_libre.py}\newline\texttt{modo\_historia.py}\newline\texttt{modo\_feedback.py} & Flujos pedagógicos en terminal \\
UI gráfica & \texttt{Juego/Grafico/} (\texttt{pantalla\_feedback.py}, \texttt{pantalla\_info.py}, \ldots) & Pantallas pygame y barra superior fija \\
Interacción & \texttt{consola.py}\newline\texttt{entrada\_teclas.py}\newline\texttt{entrada\_menu.py}\newline\texttt{navegacion.py} & Teclas (\texttt{msvcrt} en Windows), menús, pausa, ayuda \\
...
Pruebas de regresión & \texttt{python -m unittest discover -s Tests -v} (262 tests) \\
...
\textbf{Estado del entregable:} cuestionario en consola y \textbf{versión gráfica} pygame con cuatro modos operativos (libre, historia, resistencia, feedback), banco de \textbf{480 preguntas} revisadas manualmente, herramientas de mantenimiento del dataset y empaquetado opcional en ejecutable Windows. La capa narrativa escape room / novela gráfica completa queda como evolución futura.

Se entregó un cuestionario funcional en consola y en pygame:

\begin{tabular}{@{}lp{8cm}@{}}
\toprule
\textbf{Componente} & \textbf{Resultado} \\
\midrule
Lanzador consola & \texttt{Juego/juego\_consola.py} (tutorial de foco de teclado al inicio) \\
Lanzador gráfico & \texttt{Juego/juego\_grafico.py} + \texttt{Juego/Grafico/} (libre, historia, resistencia, feedback, info) \\
Paquete de lógica & \texttt{Juego/Comun/} (dominio) + \texttt{Juego/Consola/} y \texttt{Juego/Grafico/} (UI) \\
...
Pruebas & Suite en \texttt{Tests/} + \texttt{Files/test\_*.py} --- \textbf{262 tests}; CI en GitHub Actions \\
\bottomrule
\end{tabular}
...
Se desarrolló un conjunto de scripts en \texttt{Files/} con punto de entrada unificado (\texttt{mantenimiento.py}): validación, revisión, pipeline de plantillas, auditorías (salida en consola), deduplicación y estadísticas del histórico de qualificacions. La lógica de claves de contenido y expansión de plantillas se centralizó en \texttt{utils\_plantillas\_core.py}, compartida con \texttt{Juego/Comun/datos.py}. Tras el cierre del banco (2026-06), los scripts de regeneración masiva del CSV se eliminaron del repositorio. Suite de pruebas en \texttt{Tests/} (262 tests) con CI en GitHub Actions. Catálogo de comandos: \texttt{Files/README.md}.
...
OE4 & \textbf{Parcialmente cumplido} & \texttt{juego\_grafico.py}: libre, historia, resistencia y feedback; falta narrativa escape room \\
OE5 & \textbf{Parcialmente cumplido} & Banco validado, 262 tests + CI; sin estudio con usuarios \\
\bottomrule
\end{tabular}
\fintabla

Los \textbf{objetivos específicos} OE2, OE3 y OE5 están cubiertos. OE4 avanza con interfaz gráfica pygame en los cuatro modos principales; OE1 (narrativa gráfica completa tipo escape room) queda como trabajo futuro. La consola y el gráfico comparten dominio en \texttt{Juego/Comun/}.
