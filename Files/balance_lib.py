...
def ejecutar_validar(detalle: bool = False, estricto: bool = False) -> int:
    ...
    for m in msgs:
        print(f"  - {m}")
    print(
        "\nSugerencia: revisar avisos con mantenimiento.py validar o auditar-distractores; "
        "validar con python Files/mantenimiento.py validar. "
...
def _ejecutar_ajuste_intercambios(
    ...
    if ok_despues:
        ...
        if len(msgs_despues) > 12:
            print(f"  ... y {len(msgs_despues) - 12} más (python Files/mantenimiento.py validar --detalle)")
    ...
    if not ok_despues:
        print("Reordenar CSV: requiere TFG_PERMITIR_CSV=1 (banco cerrado)")
    return 0 if ok_despues else 1
...
def cargar_git_rows() -> list[dict]:
    raw = subprocess.check_output(["git", "show", "HEAD:Data/Banco/Preguntas.csv"], cwd=BASE)
...
def ejecutar_corregir() -> int:
    ...
    try:
        ...
        print("Error: no se pudo leer HEAD:Data/Banco/Preguntas.csv desde git")
        return 1
