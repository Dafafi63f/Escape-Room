...
def _generar_reemplazo_exacto(tema, plantillas, claves_existentes):
    ...
    for template in templates:
        for n in expandir_plantilla(template):
...
def ejecutar_enunciado(inplace: bool = False, output: str | None = None, seed: int = 42) -> None:
    ...
    out_path = PATH_PREGUNTAS if inplace else (BASE / (output or "Data/Banco/Preguntas_sin_duplicados_enunciado.csv"))
...
def generar_reemplazo_enunciado(
    ...
    if not templates:
        return None
    for template in templates:
        for cand in expandir_plantilla(template):
