...
CLI unificada de mantenimiento (banco cerrado 480 preguntas).
  ...
  python Files/Scripts/mantenimiento.py validar [--detalle] [--estricto]
  python Files/Scripts/mantenimiento.py revision [--estadisticas]
  python Files/Scripts/mantenimiento.py dataset [--variedad]
  python Files/Scripts/mantenimiento.py auditar-distractores [--json PATH] [--solo-dataset]
...
SCRIPTS = _SCRIPTS
FILES = _SCRIPTS.parent
ROOT = FILES.parent

if sys.platform == "win32":
    sys.stdout.reconfigure(encoding="utf-8", errors="replace")

from utils_banco_cerrado import mensaje_comando_balance_bloqueado  # noqa: E402
from utils_dataset_csv import borrar_pycache_en_proyecto  # noqa: E402

_COMANDOS_BALANCE_BLOQUEADOS = frozenset(
    {
        "ajustar",
        "reordenar",
        "ordenar-ladder",
...
def cmd_criterios(args: argparse.Namespace) -> int:
    ...
    cmd = [sys.executable, str(SCRIPTS / "exportar_criterios_clasificacion_materia.py")]
...
def cmd_balance_legacy(args: argparse.Namespace) -> int:
    print(mensaje_comando_balance_bloqueado(args.legacy_cmd))
    return 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    for nombre in sorted(_COMANDOS_BALANCE_BLOQUEADOS):
        ...
        p.set_defaults(func=cmd_balance_legacy, legacy_cmd=nombre)
