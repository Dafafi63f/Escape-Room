...
def cmd_criterios(args: argparse.Namespace) -> int:
    ...
    cmd = [sys.executable, str(FILES / "exportar_criterios_clasificacion_materia.py")]
...
def cmd_pycache(args: argparse.Namespace) -> int:
    from borrar_pycache import main as borrar_main

    forward = ["--dry-run"] if args.dry_run else []
    return borrar_main(forward)
