...
Validación y estadísticas de ``Data/Preguntas.csv`` (solo lectura).
  ...
  python Files/mantenimiento.py revision          # revisión amplia
  python Files/mantenimiento.py estadisticas      # tablas de distribución
  python Files/mantenimiento.py dataset           # checks adicionales (hash, complejidad, …)
"""
...
def validacion_extendida(df: pd.DataFrame, *, con_variedad: bool) -> int:
    ...
    rc = 0
    if con_variedad:
        print("Variedad temática: omitida (script legado eliminado tras cierre del banco)")
